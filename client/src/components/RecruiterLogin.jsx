  ...
  return (
    ...
    <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        {state!=='Login' &&  <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
            ...
            <input className='outline-none text-sm' type="text" onChange={e=>setName(e.target.value)} value={name} placeholder='Company Name'  required/>
        </div>}
       
        <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
            <img src={assets.email_icon} alt="" />
            <input className='outline-none text-sm' type="email" onChange={e=>setEmail(e.target.value)} value={email} placeholder='Emaild ID'  required/>
        </div>
        <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
